/**
 * Geocode FR End-to-End Tests
 * 
 * Comprehensive integration tests for the pricofy-geocode-fr Lambda function.
...
import { GeocodeFRClient } from './client';
...
// Initialize client
const client = new GeocodeFRClient();

describe('Geocode FR E2E Tests', () => {
  beforeAll(() => {
    console.log('🚀 Starting Geocode FR E2E Tests');
  ...
  afterAll(() => {
    console.log('\n🏁 Geocode FR E2E Tests Completed');
  });
  ...
  describe('Edge Cases - Reverse Geocode', () => {
    ...
    test('should handle coordinates outside Spain (should return error or closest)', async () => {
      ...
      // Should either return error or closest French location
      if (response.statusCode === 400) {
        ...
        expect(response.statusCode).toBe(200);
        expect(response.body.country).toBe('France');
        console.log(`   ✅ Returned closest French location`);
      }
    }, 30000);
  });
