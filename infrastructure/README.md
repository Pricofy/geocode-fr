# Pricofy Geocode FR - Infrastructure

This directory contains the AWS CDK infrastructure-as-code for deploying the **pricofy-geocode-fr** service.

## Overview

The infrastructure is defined using AWS CDK (Cloud Development Kit) and deploys:

- **Lambda Function**: Single function (`pricofy-geocode-fr-{env}`) with internal routing
...
- **Lambda Function**: `pricofy-geocode-fr-{env}`
...
From the root of `pricofy-geocode-fr`:
...
npm install
npm run build
cdk deploy GeocodeFrStack-dev --context environment=dev
```
...
CloudWatch Logs:
```bash
aws logs tail /aws/lambda/pricofy-geocode-fr-dev --follow
```
