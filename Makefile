...
SERVICE_NAME = pricofy-geocode-fr
STACK_SERVICE = PricofyGeocodeFrStack
LAMBDA_GEOCODE = pricofy-geocode-fr
...
test-geocode: ## Test geocode Lambda (ENV=dev)
	...
	@aws lambda invoke \
		...
		--payload $(shell echo '{"body":"{\"operation\":\"geocode-by-postal\",\"postalCode\":\"75001\"}"}' | base64 -w 0) \
		response.json
