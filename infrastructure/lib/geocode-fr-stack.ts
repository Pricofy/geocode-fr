...
export class GeocodeFrStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: GeocodeFrStackProps) {
    ...
    const warmupRule = new events.Rule(this, 'WarmupRule', {
      ruleName: `pricofy-geocode-fr-warmup-${props.environment}`,
      schedule: events.Schedule.rate(cdk.Duration.minutes(5)),
      description: `Warmup for geocode-fr (${props.environment})`,
    });
    ...
    const functionArn = `arn:aws:lambda:${this.region}:${this.account}:function:pricofy-geocode-fr`;
